...
class AES:
    ...
    def get_key_length(self, algo):
        if algo.count("-") != 2:
            raise ValueError("Unkown algorithm")
        try:
            return int(algo.split("-")[1]) // 8
        except ValueError:
            raise ValueError("Unkown algorithm") from None
    ...
    def new_key(self, algo="aes-256-cbc"):
        ...
        return self._backend.random(self.get_key_length(algo))
    ...
    def decrypt(self, ciphertext, iv, key, algo="aes-256-cbc"):
        ...
        key_length = self.get_key_length(algo)
